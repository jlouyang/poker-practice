...
class SharkBot(BotStrategy):
    ...
    def __init__(self, tightness: int = 55, aggression: int = 65):
        ...
        self._bluff_frequency = (100 - tightness) * 0.003 + aggression * 0.002
    ...
    def _decide_no_bet(self, state: VisibleGameState, equity: float, pot: int) -> BotAction:
        ...
        # Medium hand: sometimes bet for thin value / protection
        if equity > 0.45 and random.random() < self._aggression / 200:
            bet_size = max(state.big_blind, int(pot * 0.4))
            bet_size = min(bet_size, state.my_stack)
            return BotAction(ActionType.BET, bet_size)

        # Bluff opportunity
        if equity < 0.35 and random.random() < self._bluff_frequency:
            if state.street in (Street.TURN, Street.RIVER):
                bet_size = max(state.big_blind, int(pot * 0.6))
                bet_size = min(bet_size, state.my_stack)
                return BotAction(ActionType.BET, bet_size)
    ...
    def _decide_facing_bet(
        ...
        # Semi-bluff raise with drawing hands
        if (
            equity > pot_odds * 0.7
            and random.random() < self._bluff_frequency
            and state.street in (Street.FLOP, Street.TURN)
        ):
            raise_amount = to_call + max(state.big_blind * 3, int(pot * 0.5))
            raise_amount = min(raise_amount, state.my_stack)
            return BotAction(ActionType.RAISE, raise_amount)

        # Borderline: call with implied odds consideration
    ...
    def _size_bet(self, pot: int, equity: float, state: VisibleGameState) -> int:
        """Size bets proportional to hand strength and pot."""
        if equity > 0.85:
        ...
        bet = max(state.big_blind, int(pot * fraction))
